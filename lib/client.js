...
Client.prototype.sendRequest = function(name, payload, wantReply, callback) {
  var payloadlen = Buffer.byteLength(payload),
      namelen = Buffer.byteLength(name),
      buf = new Buffer(1 + 4 + namelen + 1 + 4 + payloadlen);

  buf[0] = MESSAGE.GLOBAL_REQUEST;

  buf.writeUInt32BE(namelen, 1, true);
  buf.write(name, 5, namelen, 'utf8');

  buf[1 + 4 + namelen] = (wantReply === undefined || wantReply === true ? 1 : 0);

  buf.writeUInt32BE(payloadlen, 1 + 4 + namelen + 1, true);
  buf.write(payload, 1 + 4 + namelen + 1 + 4, payloadlen, 'utf8');
  return this._sshstream.send(buf, callback);
};
